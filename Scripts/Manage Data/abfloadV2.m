function [d,si,h]=abfloadV2(fn,varargin)
...
%   % Added by Vlad 121216
%   ADCSection.llNumEntries=ADCSection.llNumEntries+1;
...
switch h.nOperationMode
  ...
  case {2,4,5}
    ...
    % ** load data if requested
%     appendix=[selectedSegStartInPts(end):mean(diff(selectedSegStartInPts)):selectedSegStartInPts(end)*2+mean(diff(selectedSegStartInPts))];
%     selectedSegStartInPts = [selectedSegStartInPts,appendix(2:end)];
    if doLoadData
      for i=1:nSweeps,
