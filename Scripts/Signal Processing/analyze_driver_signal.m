...
%         end
    ...
    % Thd=abs(min(d));
    SamplesDelete=1:numel(xe);
    [Apeaks,Npeaks]=findpeaks(xe);
    [~,Nvalleys]=findpeaks(-xe);
    if isempty(Nvalleys)
        Nvalleys=[1,numel(xe)];
    end
    SaveSamples=[];
    for n=1:length(Npeaks)
        if Apeaks(n)>=std(noisex)
            % Before the peak
            auxN=0;
            while and(and(xe(Npeaks(n)-auxN)>0,~ismember(Npeaks(n)-auxN,Nvalleys)),...
                    Npeaks(n)-auxN>1)
            ...
            auxN=1;
            while and(and(xe(Npeaks(n)+auxN)>0,~ismember(Npeaks(n)+auxN,Nvalleys)),...
                    Npeaks(n)+auxN<numel(x_sparse)-1)
                SaveSamples=[SaveSamples,(Npeaks(n)+auxN)];
                auxN=auxN+1;
            end
        end
